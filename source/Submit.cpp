...
{
void
submitBuffers (VulkanInstance& vulkanInstance, uint32_t& currentBufferIdx)
{
  ...
  vulkanInstance.submitInfo.pWaitDstStageMask = nullptr;
  vulkanInstance.submitInfo.pNext = nullptr;
  VK_CHECK_RESULT (vkQueueSubmit (
    vulkanInstance.queue, 1, &vulkanInstance.submitInfo, VK_NULL_HANDLE));
...
void
submitComputeBuffer (VkQueue queue,
                     ...
                     VkSemaphore computeSemaphore)
{
  ...
  VK_CHECK_RESULT (
    vkQueueSubmit (queue, 1, &computeSubmitInfo, VK_NULL_HANDLE));
}

void
submitGraphicsBuffer (VulkanInstance& vulkanInstance,
...
void
submitBuffers (VulkanInstance& vulkanInstance,
               VkCommandBuffer computeCommandBuffer,
               VkSemaphore graphicsSemaphore,
               VkSemaphore computeSemaphore)
{
  submitComputeBuffer (vulkanInstance.computeQueue,
                       computeCommandBuffer,
                       graphicsSemaphore,
                       computeSemaphore);
  submitGraphicsBuffer (vulkanInstance,
                        vulkanInstance.currentBuffer,
                        graphicsSemaphore,
                        computeSemaphore);
}
} // namespace VkVP